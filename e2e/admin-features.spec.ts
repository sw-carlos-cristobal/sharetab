...
import { users, testUsers, login, authedContext, trpcQuery, trpcMutation, trpcResult } from './helpers';
...
test.describe('User suspend/unsuspend', () => {
  test('admin page shows suspend button for non-admin users', async ({ page }) => {
    ...
    const hasSuspend = await targetRow
      .getByLabel(/^Suspend /)
      .isVisible()
      .catch(() => false);
    const hasUnsuspend = await targetRow
      .getByLabel(/^Unsuspend /)
      .isVisible()
      .catch(() => false);
  ...
  test('suspend and unsuspend user via API', async () => {
    ...
    const listData = await trpcResult(await trpcQuery(ctx, 'admin.listUsers', { search: testUsers.suspend.email }));
    const target = listData.users.find((u: { email: string }) => u.email === testUsers.suspend.email);
    ...
    // Verify suspended
    const listData2 = await trpcResult(await trpcQuery(ctx, 'admin.listUsers', { search: testUsers.suspend.email }));
    const target2 = listData2.users.find((u: { email: string }) => u.email === testUsers.suspend.email);
...
test.describe('Registration control', () => {
  ...
  test('get and set registration mode via API', async () => {
    ...
    // Get current mode
    const getData = await trpcResult(await trpcQuery(ctx, 'admin.getRegistrationMode'));
  ...
  test('create and revoke system invite via API', async () => {
    ...
    // List invites
    const invites = await trpcResult(await trpcQuery(ctx, 'admin.listSystemInvites'));
...
test.describe('Announcement banner', () => {
  ...
  test('set and clear announcement via API', async () => {
    ...
    // Get announcement
    const getData = await trpcResult(await trpcQuery(ctx, 'admin.getAnnouncement'));
...
test.describe('Global activity feed', () => {
  ...
  test('global activity API returns data', async () => {
    const ctx = await authedContext(users.alice.email, users.alice.password);
    const data = await trpcResult(await trpcQuery(ctx, 'admin.getGlobalActivity', { limit: 5 }));
    expect(data.items).toBeInstanceOf(Array);
    await ctx.dispose();
  });
});
...
test.describe('AI usage statistics', () => {
  ...
  test('AI stats API returns data', async () => {
    const ctx = await authedContext(users.alice.email, users.alice.password);
    const stats = await trpcResult(await trpcQuery(ctx, 'admin.getAIStats'));
...
test.describe('Admin tools', () => {
  ...
  test('data export API returns JSON file', async () => {
    ...
    expect(res.headers()['content-disposition']).toContain('sharetab-export');
...
test.describe('Server logs', () => {
  ...
  test('server logs API returns entries', async () => {
    const ctx = await authedContext(users.alice.email, users.alice.password);
    const data = await trpcResult(await trpcQuery(ctx, 'admin.getLogs', { limit: 50 }));
  ...
  test('server logs API supports level filtering', async () => {
    const ctx = await authedContext(users.alice.email, users.alice.password);
    const data = await trpcResult(await trpcQuery(ctx, 'admin.getLogs', { minLevel: 'warn', limit: 50 }));
    expect(data.entries).toBeInstanceOf(Array);
...
test.describe('User impersonation', () => {
  test('admin page shows impersonate button for non-admin users', async ({ page }) => {
    ...
    await expect(userSection).toBeVisible({ timeout: 30000 });
    await userSection.getByPlaceholder('Search by name or email...').fill(testUsers.suspend.email);
    const targetRow = userSection.locator('table').first().locator('tr', { hasText: testUsers.suspend.email }).first();
    await expect(targetRow).toBeVisible({ timeout: 15000 });
  ...
  test('impersonation API start and stop', async () => {
    ...
    // Get Bob's user ID
    const listData = await trpcResult(await trpcQuery(ctx, 'admin.listUsers', { search: 'bob@example.com' }));
    const bob = listData.users.find((u: { email: string }) => u.email === 'bob@example.com');
    ...
    const statusData = await trpcResult(await trpcQuery(ctx, 'admin.getImpersonationStatus'));
  ...
  test('impersonation banner appears immediately after clicking impersonate', async ({ page }) => {
    ...
    const bobRow = userSection.locator('table').first().locator('tr', { hasText: 'bob@example.com' }).first();
