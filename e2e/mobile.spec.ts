...
test.describe('Mobile Responsive', () => {
  ...
  test('mobile layout stacks cards vertically', async ({ page }) => {
    ...
    const groupA = await createTestGroup(users.alice.email, users.alice.password, [], groupAName);
    const groupB = await createTestGroup(users.alice.email, users.alice.password, [], groupBName);
