...
async function uploadAndProcess(
  ...
  const procRes = await trpcMutation(ctx, 'receipts.processReceipt', { receiptId: receiptId! }, 120000);
...
test.describe('Save for later persistence', () => {
  ...
  test('saves and restores item assignments', async () => {
    ...
    try {
      ...
      expect(savedItem1.assignments).toEqual(expect.arrayContaining([expect.objectContaining({ userId: aliceId })]));
      expect(savedItem2.assignments).toEqual(expect.arrayContaining([expect.objectContaining({ userId: bobId })]));
    } finally {
      await dispose();
    }
  });
