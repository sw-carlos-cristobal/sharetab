...
test.describe('Finalize claim session', () => {
  ...
  test('full finalize flow: claim items, save, finalize, see summary', async ({ browser }) => {
  ...
  test('API: finalizeSession calculates correct per-person totals', async () => {
    ...
    const totalSum = session.summary.reduce((sum: number, s: { total: number }) => sum + s.total, 0);
