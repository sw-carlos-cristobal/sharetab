...
import { users, login, authedContext, trpcQuery, trpcMutation, trpcResult, trpcError } from './helpers';
...
test.describe('Admin — Guest Split Cleanup', () => {
  ...
  test('getExpiredSplitCount returns counts', async () => {
    ...
    const data = await trpcResult(await trpcQuery(ctx, 'admin.getExpiredSplitCount'));
  ...
  test('admin tools section shows Guest Split Cleanup card', async ({ page }) => {
    ...
    await expect(toolsSection.getByRole('button', { name: 'Purge Expired' })).toBeVisible();
  });
