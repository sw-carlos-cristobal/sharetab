...
import { users, login, authedContext, trpcQuery, trpcMutation, trpcResult } from './helpers';
...
test.describe('Meridian auth section — unhealthy', () => {
  ...
  test('shows Meridian Authentication section with unhealthy status', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Meridian Authentication' })).toBeVisible();
    await expect(page.getByText('Claude OAuth Status')).toBeVisible();
    await expect(page.getByText(/Authentication expired|Proxy not running/i)).toBeVisible();
  });

  test('shows Authenticate with Claude button when unhealthy', async ({ page }) => {
    await expect(page.getByRole('button', { name: 'Authenticate with Claude' })).toBeVisible();
  });

  test('shows notification preferences card', async ({ page }) => {
    await expect(page.getByText('Auth Expiry Notifications')).toBeVisible();
    await expect(page.getByText('How often to receive email alerts')).toBeVisible();
    await expect(page.locator("[data-slot='select-trigger']").last()).toBeVisible();
  });

  test('notification preference dropdown has all options', async ({ page }) => {
...
test.describe('Meridian login flow', () => {
  test('authenticate button triggers login flow UI', async ({ page }) => {
    ...
    await expect(page.getByText('Click the link below to sign in with Claude.')).toBeVisible();
    await expect(page.getByText('After authorizing, copy the')).toBeVisible();
    await expect(page.getByRole('link', { name: 'Open authentication page' })).toBeVisible();
    await expect(page.getByPlaceholder('https://platform.claude.com/oauth/code/callback?code=...')).toBeVisible();
  ...
  test('submit button enables when code is entered', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Authenticate with Claude' }).click();
    await page.getByPlaceholder('https://platform.claude.com/oauth/code/callback?code=...').fill('abc123');
  ...
  test('successful login shows success message', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Authenticate with Claude' }).click();
    await page.getByPlaceholder('https://platform.claude.com/oauth/code/callback?code=...').fill('valid-code-123');
    await page.getByRole('button', { name: 'Submit' }).click();

    await expect(page.getByText('Login successful! Authentication restored.')).toBeVisible();
  ...
  test('failed login shows error with retry option', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Authenticate with Claude' }).click();
    await page.getByPlaceholder('https://platform.claude.com/oauth/code/callback?code=...').fill('bad-code');
    await page.getByRole('button', { name: 'Submit' }).click();
...
test.describe('Meridian auth section — healthy', () => {
  test('shows Authenticated status with email', async ({ page }) => {
    ...
    await expect(meridianSection.getByRole('button', { name: 'Authenticate with Claude' })).not.toBeVisible();
  });
});
...
test.describe('Meridian auth section — not applicable', () => {
  test('section is hidden when AI provider is not meridian', async ({ page }) => {
    await login(page, users.alice.email, users.alice.password);
    await page.goto('/en/admin');
...
test.describe('Meridian notify preference — API', () => {
  test('get and set notification preference via API', async () => {
    ...
    const getData = await trpcResult(await trpcQuery(ctx, 'admin.getMeridianNotifyPreference'));
    const original = getData.interval;

    const setRes = await trpcMutation(ctx, 'admin.setMeridianNotifyPreference', { interval: '6h' });
    const setBody = await setRes.json();
    expect(setBody.result.data.json.interval).toBe('6h');

    const verify = await trpcResult(await trpcQuery(ctx, 'admin.getMeridianNotifyPreference'));
