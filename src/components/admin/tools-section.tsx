...
import { Loader2, Download, Mail, Wrench, CheckCircle2, AlertCircle, Trash2 } from 'lucide-react';
      ...
      <div className="grid gap-4 @2xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2">
            <p className="text-sm text-muted-foreground">{t('tools.exportDescription')}</p>
            <Button size="sm" variant="outline" onClick={handleExport} disabled={exporting}>
              {exporting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}
              {t('tools.exportButton')}
            </Button>
            {exportResult && (
              <p className={`text-sm ${exportResult.isError ? 'text-destructive' : 'text-green-600'}`}>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <p className="text-sm text-muted-foreground">{t('tools.emailDescription')}</p>
            <Button
              size="sm"
              variant="outline"
