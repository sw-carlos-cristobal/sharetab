...
Tip: set `AUTH_RATE_LIMIT_MAX=9999`, `AUTH_IP_RATE_LIMIT_MAX=9999`, and `GUEST_RATE_LIMIT_MAX=9999` in `.env` to avoid rate limiting during test runs. Every local login counts against one IP, because Next.js adds an `x-forwarded-for` header to local requests.
